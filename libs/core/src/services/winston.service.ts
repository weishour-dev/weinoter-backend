...
export class WinstonConfigService implements WinstonModuleOptionsFactory {
  ...
  createWinstonModuleOptions(): WinstonModuleOptions {
    ...
    return {
      ...
      format: format.combine(format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), format.prettyPrint(), format.ms()),
      transports: [
        new transports.DailyRotateFile({
          ...
          auditFile: path.join(
            ...
            '..',
            'logs',
            `${this.timeUtil.dateFormat(new Date(), 'yyyy-MM-dd')}-success.json`,
          ),
        ...
        new transports.DailyRotateFile({
          ...
          auditFile: path.join(
            ...
            '..',
            'logs',
            `${this.timeUtil.dateFormat(new Date(), 'yyyy-MM-dd')}-error.json`,
          ),
        ...
        new transports.Console({
          ...
          format: format.combine(
            format.printf(info => {
              ...
              // 信息格式处理
              const messages = info.message || JSON.stringify(omit(info, ['context', 'level', 'timestamp', 'ms']));

              const message =
                this.colorUtil.hex(color, CONSOLE_TOP_LINE) + messages + this.colorUtil.hex(color, CONSOLE_BOTTOM_LINE);

              const result = `${this.colorUtil.color.bgHex(color).hex(Color.black).bold(level)} ${this.colorUtil.hex(
                Color.blue,
                info.timestamp,
