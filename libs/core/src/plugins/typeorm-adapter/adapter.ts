...
export default class TypeORMAdapter implements FilteredAdapter, UpdatableAdapter {
  ...
  private constructor(option: TypeORMAdapterOptions, adapterConfig?: TypeORMAdapterConfig) {
   ...
   */
  public static async newAdapter(option: TypeORMAdapterOptions, adapterConfig?: TypeORMAdapterConfig) {
  ...
  public async loadFilteredPolicy(model: Model, filter: FindOptionsWhere<GenericCasbinRule>) {
  ...
  public async updatePolicy(sec: string, ptype: string, oldRule: string[], newRule: string[]) {
    ...
    if (existing) await this.getRepository().save(this.getRepository().merge(existing, newline));
  }
   ...
   * @param type
   */
  private static getCasbinRuleType(type: string, adapterConfig?: TypeORMAdapterConfig): CasbinRuleConstructor {
